...
async def ensure_schema():
    """Create tables if they don't exist yet (in case this bot starts
    before hosting_panel_bot.py has run once and created them)."""
...
async def claim_pending(bot_id: str) -> Optional[Dict]:
    ...
    between this bot and the hosting bot's own approve/reject buttons,
    which share the same DB) can only ever be acted on once."""
    async with aiosqlite.connect(DATABASE_PATH) as conn:
        conn.row_factory = aiosqlite.Row
        cur = await conn.execute("SELECT * FROM pending_deploys WHERE bot_id=?", (bot_id,))
...
async def execute_approval(bot_id: str, admin_id: int, approve: bool) -> bool:
    ...
    try:
        ...
        if not staging.exists():
            ...
            # attempt crashed after the move but before it could record
            # success — don't fabricate a false "rejected" over a bot that
            # is actually sitting deployed on disk.
...
def main():
    ...
    app = Application.builder().token(APPROVAL_BOT_TOKEN).build()
