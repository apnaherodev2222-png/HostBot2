...
OWNER_ID = int(os.getenv("OWNER_ID", "7544628719"))
ADMIN_ID = 7544628719
YOUR_USERNAME = "DarkTechZone0"
UPDATE_CHANNEL = "https://t.me/DarkTechZone2"
...
async def migrate_db():
    ...
    try:
        async with aiosqlite.connect(DATABASE_PATH) as conn:
            await conn.execute("PRAGMA table_info(user_files)")
            columns = [row[1] for row in await conn.fetchall()]
            ...
            await conn.execute("PRAGMA table_info(active_users)")
            columns = [row[1] for row in await conn.fetchall()]
...
async def init_db():
    ...
    try:
        async with aiosqlite.connect(DATABASE_PATH) as conn:
            ...
            # Shared with approval_bot.py — created here too (IF NOT EXISTS) so
            # start order between the two processes never matters.
...
def is_free_tier(user_id):
    """Owner/admin/premium users are trusted and skip the approval queue —
    matches approval_bot.py's docstring ('auto-moderation of free-tier uploads')."""
...
async def get_mute_remaining(user_id):
    """Returns remaining mute duration (timedelta) if the user is currently
    muted (set by approval_bot.py after a rejected upload), else None."""
    async with aiosqlite.connect(DATABASE_PATH) as conn:
        cursor = await conn.execute('SELECT mute_until FROM muted_users WHERE user_id = ?', (user_id,))
...
async def get_approval_status(user_id, file_name):
    """Returns (status, reason) for a file pending/decided in the approval
    queue, or (None, None) if no such row exists (e.g. trusted user whose
    upload never went through the queue)."""
    async with aiosqlite.connect(DATABASE_PATH) as conn:
        cursor = await conn.execute(
            'SELECT status, reason FROM pending_approvals WHERE user_id = ? AND file_name = ?',
            (user_id, file_name)
        )
        row = await cursor.fetchone()
...
async def approval_result_notifier():
    """Background loop in the MAIN bot process: watches pending_approvals for
    rows approval_bot.py (or the admin panel) has just decided on and DMs the
    uploader via THIS bot. Runs here rather than in approval_bot.py because
    the uploader has definitely /start'd this bot (they used it to upload)
    but very likely never started the separate approval bot."""
    while True:
        try:
            async with aiosqlite.connect(DATABASE_PATH) as conn:
                cursor = await conn.execute(
                    "SELECT user_id, file_name, status, reason FROM pending_approvals "
                    "WHERE status IN ('approved', 'rejected') AND notified = 0"
                )
                rows = await cursor.fetchall()
                for user_id, file_name, status, reason in rows:
                    try:
                        if status == 'approved':
                            text = f"✅ Your file <code>{file_name}</code> passed review and can now be run!"
                        else:
                            text = (f"❌ Your file <code>{file_name}</code> was rejected: {reason}\n\n"
                                    f"⏳ You're temporarily muted from uploading for 2 hours.")
                        await bot.send_message(user_id, text, parse_mode="HTML")
                    except Exception as e:
                        logger.warning(f"Could not notify {user_id} about {file_name}: {e}")
                    await conn.execute(
            ...
            logger.error(f"approval_result_notifier error: {e}", exc_info=True)
...
async def cmd_start(message: types.Message):
    ...
    try:
        async with aiosqlite.connect(DATABASE_PATH) as conn:
            ...
            await conn.execute('INSERT OR REPLACE INTO active_users (user_id, join_date, last_active) VALUES (?, ?, ?)', 
                              (user_id, now, now))
            await conn.commit()
    except Exception as e:
...
async def handle_document(message: types.Message):
    ...
    if is_free_tier(user_id):
        remaining = await get_mute_remaining(user_id)
        if remaining:
            mins = int(remaining.total_seconds() // 60) + 1
            await message.answer(f"⏳ You're muted from uploading for another {mins} minute(s) due to a rejected script.")
            return
    ...
    try:
        ...
        needs_approval = file_ext != '.zip' and is_free_tier(user_id)

        if needs_approval:
            async with aiosqlite.connect(DATABASE_PATH) as conn:
                await conn.execute(
                    'INSERT OR REPLACE INTO pending_approvals (user_id, file_name, status, notified) VALUES (?, ?, ?, 0)',
                    (user_id, file_name, 'pending')
                )
                await conn.commit()
        ...
        if file_ext == '.zip':
            ...
            # No "Run Now" button here on purpose — run_script also re-checks
            # status server-side, but hiding the button avoids a confusing
            # "under review" alert on tap.
            keyboard = InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="⭐ Add Favorite", callback_data=f"toggle_fav:{file_name}")],
                [InlineKeyboardButton(text="ℹ️ File Info", callback_data=f"file_info:{file_name}"),
                 InlineKeyboardButton(text="🗑️ Delete", callback_data=f"delete_file:{file_name}")],
        ...
        status_note = "\n🕵️ <b>Under review</b> — you'll get a message here once it's approved to run.\n" if needs_approval else ""
...
async def callback_run_script(callback: types.CallbackQuery):
    ...
    if is_free_tier(user_id) and file_path.suffix.lower() in ('.py', '.js'):
        status, reason = await get_approval_status(user_id, file_name)
        if status != 'approved':
            if status == 'rejected':
                await callback.answer(f"❌ This file was rejected: {reason}", show_alert=True)
            else:
                # status is 'pending' (normal case) or None (e.g. a file
                # uploaded before this approval flow existed) — either way,
                # don't run it until it has an explicit 'approved' row.
                await callback.answer("🕵️ Still under review. You'll be notified once it's approved.", show_alert=True)
            return
    ...
    try:
        ...
        # Acquire semaphore before starting process to limit concurrency
        ...
        if 'script_semaphore' in locals() and script_semaphore.locked():
            script_semaphore.release()
...
async def callback_stop_script(callback: types.CallbackQuery):
    ...
    try:
        ...
        # Release the semaphore slot back
        script_semaphore.release()
...
async def cmd_ban_user(message: types.Message):
    ...
    try:
        ...
        async with aiosqlite.connect(DATABASE_PATH) as conn:
            await conn.execute('INSERT OR REPLACE INTO banned_users (user_id, banned_date, reason) VALUES (?, ?, ?)',
